...
/**
 * Upload file (PDF, dokumen, dll) ke Cloudinary menggunakan /raw/upload
 * @param {File} file - File yang akan diupload (PDF, docx, dll)
 * @param {string} folder - Folder tujuan di Cloudinary
 * @returns {{ url: string, publicId: string, path: string }}
 */
export async function uploadFile(file, folder = 'desa-baula') {
  if (!file) throw new Error('File tidak ditemukan')
  if (!CLOUD_NAME) throw new Error('NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME belum diisi di .env.local')
  if (!UPLOAD_PRESET) throw new Error('NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET belum diisi di .env.local')
  ...
  return {
    url: data.secure_url,      // URL file yang bisa langsung dipakai
    publicId: data.public_id,  // ID untuk keperluan delete nanti
    path: data.public_id,      // Alias path
  }
}

/**
 * Hapus file (non-gambar) dari Cloudinary
 * @param {string} publicId
 */
export async function deleteFile(publicId) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Gagal menghapus file dari Cloudinary')
    }
  } catch (error) {
    console.warn('Error menghapus file:', error)
  }
}
