...
export default function EditRW() {
  ...
  useEffect(() => {
    const fetchRWData = async () => {
      ...
      try {
        const rwDoc = await getDoc(doc(db, 'data_rw', id))
        if (rwDoc.exists()) {
          const data = rwDoc.data()
          setNama(data.nama || '')
          setLokasi(data.lokasi || '')
          setTotalPenduduk(data.total_penduduk ? data.total_penduduk.toString() : '')
        } else {
          alert('Data RW tidak ditemukan')
          router.push('/admin/data')
        }
      } catch (error) {
        console.error('Error fetching RW data:', error)
        alert('Gagal mengambil data RW')
        router.push('/admin/data')
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'data_rw', id), rwDataToSave)
      ...
      alert('Data RW berhasil diperbarui!')
      router.push('/admin/data')
    } catch (error) {
      console.error('Error updating RW data:', error)
      alert('Gagal memperbarui data RW')
    }
    setLoading(false)
  }
        ...
        <div className="p-8">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h1 className="text-2xl font-bold text-green-700">Edit Data RW</h1>
                <p className="text-gray-600 mt-1">Edit data RW yang sudah ada</p>
